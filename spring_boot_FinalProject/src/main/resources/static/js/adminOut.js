...
$(document).ready(function(){
	...
	$(document).mouseup(function (e){
		...
		if(dropdownContainer.has(e.target).length === 0) {
			...
			if(e.target.className != "dpBox-div") {
				if(e.target.className == "dp-all") { 
					...
					$("#chk_search").val("2");
					dropdownContainer.hide();
				} else if(e.target.className == "dp-levDate") {
					$("#dpBox-name").text("탈퇴일");
					$("#chk_search").val("3");
					dropdownContainer.hide();
				}			
   	...
   	$(".active").click(function(e) {
   		...
   		var submitPath = "/adminOutSearch/" + pageNum;
	...
	$('#allCheck').click(function() {
		if($(this).is(':checked')){	// 체크되었을 때
			...
			$(".agree_subcheck:checked").each(function() {
				arrLevId.push($(this).val());
				
			});
			// console.log(arrLevId);
		} else {
			$('.agree_subcheck').prop('checked', false);
			
			// 변수값 제거
			$(".agree_subcheck").each(function() {
				arrLevId.shift($(this).val());
			});
		}
	});
	...
	$(".agree_subcheck").on('click', function() {
		...
		if($(this).is(':checked')) {
			...
			arrLevId = arrLevId.filter((e) => e !== $(this).val());
			$('#allCheck').prop('checked', false);
		}
	});
	
	// 삭제 기능
	$("#adminOutDel").on('click', function() {
 		...
 		if(arrLevId.length == 0) {
 			...
 			if(confirm("삭제하겠습니까")) {
		 		$.ajax({
		 			type:"post",
		 			url:"/adminDeleteOut",
		 			data:{ "levIds": JSON.stringify(arrLevId) },
		 			dataType:"text",
					success:function(result){
						// 성공 시 결과 받음
						if(result == "SUCCESS") {
							alert("삭제되었습니다.");
							location.href = "/adminOutSearch/0";
						}
					},
					error:function(){
						// 오류있을 경우 수행 되는 함수
